...
module.exports = class Previous extends Button {
  ...
  async run(client, interaction, player, data, color) {
    ...
    if (message) {
      ...
      if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.length) await player.play();
      ...
      let nowplaying = new EmbedBuilder()
        ...
        .setImage(icon)
        .setFooter({ text: `${interaction.member.user.tag} skipped the track.`, iconURL: interaction.member.displayAvatarURL({ dynamic: true }) });
