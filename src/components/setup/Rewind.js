...
module.exports = class Rewind extends Button {
  constructor(client) {
    super(client, {
      id: 'REWIND_BUTTON',
    });
  ...
  async run(client, interaction, player, data, color) {
    ...
    if (message) {
      ...
      let rewindposition = Number(player.shoukaku.position) - 10000;
      const embed100 = new EmbedBuilder()
        .setColor(client.config.color)
        .setDescription(`${client.config.emojis.error} You cannot rewind more than the duration of the track.`)
      if (rewindposition < 0) return interaction.reply({ embeds: [embed100], ephemeral: true });
      const embed101 = new EmbedBuilder()
        .setColor(client.config.color)
        .setDescription(`${client.config.emojis.error} You cannot rewind while the player is paused.`)
      if (player.paused) return interaction.reply({ embeds: [embed101], ephemeral: true });
      const embed102 = new EmbedBuilder()
        .setColor(client.config.color)
        .setDescription(`${client.config.emojis.error} You cannot rewind this track.`)
      if (!player.queue.current.isSeekable) return interaction.reply({ embeds: [embed102], ephemeral: true });

      player.shoukaku.seekTo(rewindposition);
      let fields = []
      fields.push(
        {
          name: "Queued Track(s)",
      ...
      let nowplaying = new EmbedBuilder()
        ...
        .setImage(icon)
        .setFooter({ text: `${interaction.member.user.tag} rewinded the track.`, iconURL: interaction.member.displayAvatarURL({ dynamic: true }) });
